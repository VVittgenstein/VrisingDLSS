...
Latest local validation: the C# plugin and native bridge build successfully, the native export table includes the diagnostic probes through API version 7, the render-thread/D3D11/runtime probes pass locally, and a local SDK-wrapper research build passes Stage 6 DLSS init/capability query plus Stage 7 DLSS feature create/release. Stage 8A now passes in a 75-second scripted local run through engine-owned `RenderGraphResourceRegistry.GetTexture(TextureHandle&)` postfix aggregation: `CameraColor`, `Apply Exposure Destination`, `CameraDepthStencil`, and `Motion Vectors` were validated as same-device D3D11 textures at `720x480`. This still does not evaluate DLSS or prove image correctness. Release-safe builds still do not link or bundle NVIDIA SDK/runtime files. The Thunderstore zip contains only this project's own binaries and metadata. See [local runtime staging](docs/development/local-runtime-staging-2026-06-05.md).
