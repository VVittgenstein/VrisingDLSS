...
$summary = [pscustomobject]@{
    ...
    NextRecommendation = if ($mvpReady) {
        ...
        "Pass -GamePath to include local runtime evidence. Current MVP next step is not another broad DLSS search or unchanged RenderGraph map run: the native-renderfunc-entry preflight plus menu/gameplay proofs passed. Design a separately default-off native-entry argument/resource preflight, menu-first, with no command-buffer access and no DLSS evaluate."
