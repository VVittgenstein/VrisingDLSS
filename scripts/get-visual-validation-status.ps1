...
if (@($issues | Where-Object { $_ -like "Candidate * regressed*" -or $_ -like "Candidate P95 frame time worsened*" }).Count -gt 0) {
    $nextRecommendation = "Cached-driver real-evaluate crashed in nvwgf2umx.dll even with GetTexture evaluate/output-follow-up at zero. The safe CompileRenderGraph map path has pass-list, declarations, pass-data, renderfunc-metadata, and compiled-pass-info proof; execute-delegate was stable but silent; native-renderfunc-entry preflight plus menu/gameplay proofs passed, native-renderfunc-args menu/gameplay proofs passed, native-renderfunc-resource-identity menu/gameplay proofs passed, native-renderfunc-resource-tuple menu/gameplay proofs passed, native-renderfunc-resource-resolve menu/gameplay proofs passed, and native-renderfunc-resource-native-pointer menu/gameplay proofs passed. The next narrow step is the default-off no-native hdrp-postprocess-boundary proof over RenderPostProcess/DoDLSSPass/CustomPostProcessPass and concrete ProjectM custom postprocess Render methods; do not combine command-buffer access, D3D11 validation, native texture dereference, or DLSS evaluate with this boundary proof. Do not patch generated render funcs through Harmony or rerun completed/rejected probes unchanged."
}
