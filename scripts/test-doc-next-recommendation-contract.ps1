...
foreach ($path in @($visualScript, $mvpPath, $installPath, $contextPath)) {
    Add-Check `
...
if (Test-Path -LiteralPath $visualScript -PathType Leaf) {
    try {
        ...
        Add-Check `
            -Name "VisualGateNextRecommendationUsesContractBindRoute" `
            -Passed ((Test-ContainsText -Text $visualStatus.NextRecommendation -Needle "Do not rerun the same EASU ctx.cmd") `
            ...
            -Failure "visual gate next recommendation no longer points away from unchanged user-rendering and toward contract-bind"
    } catch {
        Add-Check `
...
Add-Check `
    -Name "MvpDocMarksUserRenderingAsKnownRegressed" `
    -Passed ((Test-ContainsText -Text $mvpText -Needle "known-regressed route") `
        ...
        -and (Test-ContainsText -Text $mvpText -Needle "hdrp-dlss-contract-bind-render-scale") `
        -and (Test-ContainsText -Text $mvpText -Needle "ChangeCount=0")) `
    -Evidence $mvpPath `
    -Failure "docs/mvp.md must keep the current visual gate as blocked on a known-regressed user-rendering route and name contract-bind as next runtime proof"

Add-Check `
    -Name "InstallDocTopStatusDoesNotPromoteUnchangedUserRenderingRerun" `
    -Passed ((Test-ContainsText -Text $installText -Needle "performance-regressed") `
        ...
        -and (Test-ContainsText -Text $installText -Needle "hdrp-dlss-contract-bind-render-scale")) `
    -Evidence $installPath `
    -Failure "docs/install.md top status must name the user-rendering regression and current contract-bind next proof"
...
Add-Check `
    ...
    -Passed ((Test-ContainsText -Text $installText -Needle 'The visual helper can still capture `dlss-user-rendering` when intentionally reproducing or investigating that candidate') `
        ...
        -and (Test-ContainsText -Text $installText -Needle 'Use Computer Use to click Continue/`11111` once, send no movement keys')) `
    -Evidence $installPath `
...
Add-Check `
    ...
    -Passed ((Test-ContainsText -Text $contextText -Needle "scripts\test-doc-next-recommendation-contract.ps1") `
        ...
        -and (Test-ContainsText -Text $contextText -Needle "hdrp-dlss-contract-bind-render-scale")) `
