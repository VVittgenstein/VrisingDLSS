...
internal static class DlssInitQueryProbe
{
    ...
    internal static void Run(
        ...
        ulong applicationId)
    {
        log.LogInfo("Running DLSS init/query probe.");
        ...
        {
            log.LogWarning("UnityEngine.RenderTexture type was not found. DLSS init/query probe cannot run.");
            return;
        }
        ...
        try
        {
            ...
            {
                log.LogWarning("Could not create a temporary UnityEngine.RenderTexture for DLSS init/query probing.");
                return;
            }

            TrySetName(renderTexture, "VrisingDLSS DLSS init/query probe");
            ...
            log.LogInfo($"DLSS init/query temporary RenderTexture native pointer: 0x{nativeTexturePtr.ToInt64():X}");
            ...
            var success = bridge.ProbeDlssInitQuery(nativeTexturePtr, runtimePath, applicationDataPath, applicationId);
            var status = bridge.GetDlssInitQueryStatus();
            if (success)
            {
                log.LogInfo($"DLSS init/query probe succeeded: {status}");
            }
            else if (status.IndexOf("SDK wrapper integration required", StringComparison.OrdinalIgnoreCase) >= 0)
            {
                log.LogWarning($"DLSS init/query probe blocked: {status}");
            }
            else
            {
                log.LogWarning($"DLSS init/query probe failed: {status}");
            }
        }
        catch (Exception ex)
        {
            log.LogWarning($"DLSS init/query probe failed: {GetExceptionMessage(ex)}");
        }
        finally
        {
