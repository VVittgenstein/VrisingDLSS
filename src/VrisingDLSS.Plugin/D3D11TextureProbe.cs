...
internal static class D3D11TextureProbe
{
    ...
    internal static void Run(ManualLogSource log, NativeBridge bridge)
    {
        ...
        {
            log.LogWarning("UnityEngine.RenderTexture type was not found. D3D11 texture probe cannot run.");
            return;
        }
        ...
        try
        {
            ...
            {
                log.LogWarning("Could not create a temporary UnityEngine.RenderTexture for D3D11 probing.");
                return;
            }

            TrySetName(renderTexture, "VrisingDLSS D3D11 texture probe");
            ...
            log.LogInfo($"Temporary RenderTexture native pointer: 0x{nativeTexturePtr.ToInt64():X}");
            ...
            var success = bridge.ProbeD3D11Texture(nativeTexturePtr);
            var status = bridge.GetD3D11ProbeStatus();
            if (success)
            {
                log.LogInfo($"D3D11 texture pointer probe succeeded: {status}");
            }
            else
            {
                log.LogWarning($"D3D11 texture pointer probe failed: {status}");
            }
        }
        catch (Exception ex)
        {
            log.LogWarning($"D3D11 texture pointer probe failed: {GetExceptionMessage(ex)}");
        }
        finally
        {
