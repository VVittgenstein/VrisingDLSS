...
internal sealed class ModConfig
{
    ...
    internal ConfigEntry<bool> EnableNativeRenderFuncCommandBufferDlssVisibleWritebackProbe { get; }
    ...
    internal ModConfig(ConfigFile config)
    {
        ...
        EnableNativeRenderFuncCommandBufferDlssVisibleWritebackProbe = config.Bind("Diagnostics", "EnableNativeRenderFuncCommandBufferDlssVisibleWritebackProbe", false, "High-risk native RenderGraph render-func DLSS visible write-back preflight. Carries the focused frame descriptor through ctx.cmd plugin events, evaluates DLSS into the visible EASU output, then shuts down after the target success count. Local/private image-correctness testing only.");
