...
For the normal-user candidate route, pass `-CandidateStage dlss-user-rendering`. This waits for a `DLSS user rendering evaluate succeeded` log line before screenshot/FPS capture, and it does not enable the Stage 10A hold/proof loop. MVP validation should keep V Rising's built-in FSR disabled and rely on the mod-owned render-scale/upscale path. For local/private gameplay fixtures, also pass `-ProtectSave -SaveDir ...` so the helper backs up the save before launch, archives the changed after-run state, restores the backup during cleanup, and reports the final save `ChangeCount`. A protected-save restore failure makes the helper exit nonzero after emitting its result.

```powershell
powershell -ExecutionPolicy Bypass -File scripts\run-vrising-visual-comparison.ps1 -GamePath "C:\path\to\VRising" -CandidateStage dlss-user-rendering -FsrMode Off -DurationSeconds 240 -CaptureAtSeconds 170 -CapturePerformance:$true -WaitForUserRendering:$true -DlssRuntimePath "C:\path\to\nvngx_dlss.dll" -ProtectSave -SaveDir "C:\Users\you\AppData\LocalLow\Stunlock Studios\VRising\CloudSaves\<steam-id>\v4\<save-id>"
```
