...
Current status: experimental scaffold. `DLSS.EnableDLSS=true` now starts the source-guided EASU `ctx.cmd` command-buffer candidate and has protected gameplay proof, but the package is not ready for public gameplay use until visual/performance, resize/reset, fallback, and runtime distribution gates pass.
...
`DLSS.EnableDLSS=true`, or helper stage `dlss-user-rendering`, is the current normal-user-path candidate. It does not enable the Stage 10A hold/proof loop or the old hot `RenderGraph.GetTexture` user-rendering path. It installs source-guided render-scale/HDRP EASU correlation, uses the focused EASU `ctx.cmd` command-buffer callback, keeps one native frame sequence alive, and evaluates into the visible EASU output. Release-safe builds without NVIDIA SDK wrapper support are expected to report blocked/fallback rather than crashing.
