...
Current status: experimental scaffold. `DLSS.EnableDLSS=true` can start the source-guided EASU `ctx.cmd` command-buffer candidate and has protected gameplay proof, but the latest paired user-rendering evidence is performance-regressed. Do not treat `dlss-user-rendering` as the next runtime proof to rerun unchanged. The current next proof is the protected `hdrp-dlss-contract-bind-render-scale` stage, followed by a bounded no-write cost proof only if it binds HDRP depth/motion to the engine-owned chain. The package is not ready for public gameplay use until visual/performance, resize/reset, fallback, and runtime distribution gates pass.
...
Current readiness may instead recommend the protected contract-bind proof:
...
Use Computer Use to click Continue/`11111` once, send no movement keys, then stop with `scripts\stop-vrising-automation-session.ps1` and analyze the archived log with `scripts\analyze-hdrp-dlss-schedule-audit.ps1`.
