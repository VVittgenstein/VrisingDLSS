...
Current status: experimental scaffold. `DLSS.EnableDLSS=true` can start the source-guided EASU `ctx.cmd` command-buffer candidate and has protected gameplay proof, but the latest paired user-rendering evidence is performance-regressed. Do not treat `dlss-user-rendering` as the next runtime proof to rerun unchanged. The protected `hdrp-dlss-contract-bind-render-scale` contract-bind proof has passed, so the current next proof is bounded no-write B/C/D cost isolation: EASU carrier-only, native D3D11 resource-desc validate-only, and empty existing command-buffer plugin-event callback. The package is not ready for public gameplay use until visual/performance, resize/reset, fallback, and runtime distribution gates pass.
...
powershell -ExecutionPolicy Bypass -File scripts\write-diagnostic-config.ps1 -GamePath "C:\path\to\VRising" -Stage native-renderfunc-commandbuffer-frame-descriptor-render-scale
powershell -ExecutionPolicy Bypass -File scripts\write-diagnostic-config.ps1 -GamePath "C:\path\to\VRising" -Stage native-renderfunc-commandbuffer-frame-descriptor-d3d11-render-scale
powershell -ExecutionPolicy Bypass -File scripts\write-diagnostic-config.ps1 -GamePath "C:\path\to\VRising" -Stage native-renderfunc-commandbuffer-dlss-scratch-evaluate-render-scale -DlssRuntimePath "C:\path\to\nvngx_dlss.dll" -DlssApplicationId "0"
powershell -ExecutionPolicy Bypass -File scripts\write-diagnostic-config.ps1 -GamePath "C:\path\to\VRising" -Stage native-renderfunc-commandbuffer-dlss-persistent-scratch-evaluate-render-scale -DlssRuntimePath "C:\path\to\nvngx_dlss.dll" -DlssApplicationId "0"
powershell -ExecutionPolicy Bypass -File scripts\write-diagnostic-config.ps1 -GamePath "C:\path\to\VRising" -Stage native-renderfunc-commandbuffer-dlss-visible-writeback-render-scale -DlssRuntimePath "C:\path\to\nvngx_dlss.dll" -DlssApplicationId "0"
powershell -ExecutionPolicy Bypass -File scripts\write-diagnostic-config.ps1 -GamePath "C:\path\to\VRising" -Stage native-renderfunc-commandbuffer-dlss-create-render-scale -DlssRuntimePath "C:\path\to\nvngx_dlss.dll" -DlssApplicationId "0"
...
Supported stages are `loader`, `native`, `harmony-call`, `render-thread`, `d3d11`, `frame-resource`, `upscaler-state`, `render-scale-control`, `easu-carrier-only-cost-render-scale`, `hdrp-postprocess-render-args`, `hdrp-postprocess-render-args-global-textures`, `hdrp-postprocess-render-args-render-scale`, `hdrp-postprocess-render-args-global-textures-render-scale`, `hdrp-easu-input-output-correlation-render-scale`, `native-renderfunc-resource-tuple`, `native-renderfunc-resource-tuple-render-scale`, `native-renderfunc-resource-resolve`, `native-renderfunc-resource-resolve-render-scale`, `native-renderfunc-resource-native-pointer`, `native-renderfunc-resource-native-pointer-render-scale`, `native-renderfunc-resource-d3d11-render-scale`, `native-renderfunc-context-render-scale`, `native-renderfunc-commandbuffer-event-render-scale`, `native-renderfunc-commandbuffer-payload-render-scale`, `native-renderfunc-commandbuffer-frame-descriptor-render-scale`, `native-renderfunc-commandbuffer-frame-descriptor-d3d11-render-scale`, `native-renderfunc-commandbuffer-dlss-scratch-evaluate-render-scale`, `native-renderfunc-commandbuffer-dlss-persistent-scratch-evaluate-render-scale`, `native-renderfunc-commandbuffer-dlss-visible-writeback-render-scale`, `native-renderfunc-commandbuffer-dlss-create-render-scale`, `dlss-runtime`, `dlss-init-query`, `dlss-optimal-settings`, `dlss-feature-create`, `dlss-evaluate-inputs`, `dlss-super-resolution-inputs`, `dlss-super-resolution-evaluate`, `dlss-super-resolution-persistent-evaluate`, `dlss-super-resolution-frame-sequence`, `dlss-visible-writeback`, `dlss-user-rendering`, `dlss-evaluate`, `dlss-persistent-evaluate`, and `dlsspass-resource`.
...
The visual helper can still capture `dlss-user-rendering` when intentionally reproducing or investigating that candidate, but the latest evidence says not to rerun the same EASU `ctx.cmd` user-rendering shape unchanged as the next MVP step. That route waits for a `DLSS user rendering evaluate succeeded` log line before screenshot/FPS capture, and it does not enable the Stage 10A hold/proof loop. MVP validation should keep V Rising's built-in FSR disabled and rely on the mod-owned render-scale/upscale path, but the current next runtime proof is bounded no-write B/C/D cost isolation, not another unchanged `dlss-user-rendering` visual comparison or another unchanged contract-bind run. For the known local/private `11111` gameplay fixture, also pass `-ProtectSave -SaveName 11111` so the helper resolves the CloudSaves path, backs up the save before launch, archives the changed after-run state, restores the backup during cleanup, and reports the final save `ChangeCount`. A protected-save restore failure makes the helper exit nonzero after emitting its result. Manual `-ProtectSave -SaveDir ...` remains available as a fallback for other fixtures.

```powershell
powershell -ExecutionPolicy Bypass -File scripts\run-vrising-visual-comparison.ps1 -GamePath "C:\path\to\VRising" -CandidateStage dlss-user-rendering -FsrMode Off -DurationSeconds 240 -CaptureAtSeconds 170 -CapturePerformance:$true -WaitForUserRendering:$true -DlssRuntimePath "C:\path\to\nvngx_dlss.dll" -ProtectSave -SaveName 11111
```

For an operator-controlled capture, use `-ManualCapture` and create the ready file after entering the target local/private gameplay scene. The helper removes the ready file before each run, waits for it, and does not capture before `-CaptureAtSeconds`. Stage 10A candidate runs wait for `sequenceSuccesses=30/30` by default while `KeepDlssVisibleWritebackProbeRunning=true` keeps the diagnostic candidate evaluating until cleanup. User-rendering candidate runs wait for a successful user-rendering evaluate by default. `-FsrMode Off` forces V Rising's built-in FSR setting off before the paired run, then restores the previous value during cleanup. `-FsrMode Performance` is reserved for explicitly labeled transition diagnostics that need HDRP to expose an input-smaller-than-output SR tuple.
...
When coordinating a tester-launched game or checking whether Codex can see the game before capture, run the lightweight visibility preflight. It does not launch or modify the game. Continue only when `Status` is `VisibleGameWindow`; `Missing` or `ProcessOnly` means the capture would likely fail or grab the wrong window. The visual comparison helper also records this preflight immediately before each screenshot as `VisibilityPreflightStatus` and related fields.
...
The protected contract-bind proof is now recorded in `docs/development/hdrp-dlss-contract-bind-render-scale-gameplay-result-2026-06-08.md`. Its command shape remains here only for reproduction or audit, not as the next unchanged proof to rerun:

```powershell
powershell -ExecutionPolicy Bypass -File scripts\start-vrising-automation-session.ps1 -GamePath "C:\path\to\VRising" -Stage hdrp-dlss-contract-bind-render-scale -ArtifactLabel "contract-bind-1080p" -SetClientResolution -SetClientWindowMode -ClientWindowMode 3 -Width 1920 -Height 1080 -ProtectSave -SaveName 11111
```

Use Computer Use to click Continue/`11111` once, send no movement keys, then stop with `scripts\stop-vrising-automation-session.ps1` and analyze the archived log with `scripts\analyze-hdrp-dlss-schedule-audit.ps1`. Normal forward progress should now use the B/C/D no-write cost layers under the same protected fixture.
...
`VrisingDLSS.cfg` exposes the planned MVP DLSS and advanced configuration keys, including `DLSS.EnableDLSS`, `DLSS.QualityMode`, `DLSS.PresetMode`, `DLSS.UseOfficialHdrpFeatureFlags`, `Advanced.RenderScaleOverride`, and `Advanced.MipBiasOverride`. In the current package, `DLSS.EnableDLSS=true` starts an experimental user-rendering candidate on the source-guided HDRP EASU `RenderGraphContext.cmd` boundary. It keeps broad `RenderGraph.GetTexture` discovery disabled, carries the focused EASU/HDRP frame descriptor through native plugin events, and evaluates DLSS into the visible EASU output through the local SDK-wrapper native path. This is still not public-MVP ready; image quality, performance, resize/reset behavior, and release-safe native/runtime distribution remain open.
...
`DLSS.EnableDLSS=true`, or helper stage `dlss-user-rendering`, remains an experimental normal-user-path candidate for reproduction and investigation, but current performance evidence shows it is not the next runtime proof to rerun unchanged. It does not enable the Stage 10A hold/proof loop or the old hot `RenderGraph.GetTexture` user-rendering path. It installs source-guided render-scale/HDRP EASU correlation, uses the focused EASU `ctx.cmd` command-buffer callback, keeps one native frame sequence alive, and evaluates into the visible EASU output. The next proof route is bounded no-write B/C/D cost isolation after the passing contract-bind proof; release-safe builds without NVIDIA SDK wrapper support are expected to report blocked/fallback rather than crashing.
...
`Diagnostics.EnableRenderGraphPassDataSnapshotProbe=true` plus HDRP postprocess/global texture and render-scale probes, or helper stage `easu-carrier-only-cost-render-scale`, is the bounded no-write matrix B layer. It is intentionally lighter than `hdrp-dlss-contract-bind-render-scale`: no native bridge, no NGX runtime, no DLSS evaluate, no visible write-back, no broad `RenderGraph.GetTexture`, and no heavy RenderGraph pass-list/declaration/metadata probes.

`Diagnostics.EnableNativeRenderFuncContextProbe=true`, or helper stage `native-renderfunc-context-render-scale`, is a source-guided EASU RenderGraph boundary preflight. It reuses the focused native render-func entry detour, wraps the proven `RenderGraphContext` pointer, and reads `ctx.cmd` identity only. It does not issue command-buffer work, resolve textures, or evaluate DLSS.

`Diagnostics.EnableNativeRenderFuncCommandBufferEventProbe=true`, or helper stage `native-renderfunc-commandbuffer-event-render-scale`, is the follow-up no-op command-buffer timing proof. It issues one native no-op plugin event through the focused EASU `ctx.cmd` boundary and verifies that the native render-event callback count advances. It does not pass texture resources, validate D3D11 resources, or evaluate DLSS.

`Diagnostics.EnableNativeRenderFuncCommandBufferPayloadProbe=true`, or helper stage `native-renderfunc-commandbuffer-payload-render-scale`, is the next source-guided payload/lifecycle proof. It records the focused EASU source/output native texture pointers as a pending native payload, issues one plugin event through the same EASU `ctx.cmd` boundary, and verifies that the native callback consumes the payload as a same-device upscaled pair. It does not load NGX, evaluate DLSS, or write visible output.

`Diagnostics.EnableNativeRenderFuncCommandBufferFrameDescriptorProbe=true`, or helper stage `native-renderfunc-commandbuffer-frame-descriptor-render-scale`, is the release-safe four-resource frame-descriptor proof. It carries focused EASU source/output plus HDRP depth/motion native texture pointers through one `ctx.cmd` plugin event and records descriptor metadata only. It does not query D3D11 resources, load NGX, evaluate DLSS, run user rendering, or write visible output.

`Diagnostics.EnableNativeRenderFuncCommandBufferFrameDescriptorD3D11Probe=true`, or helper stage `native-renderfunc-commandbuffer-frame-descriptor-d3d11-render-scale`, is the release-safe follow-up D3D11 validation proof for the same four-resource descriptor. It validates D3D11 texture/device/dimension shape at the focused EASU `ctx.cmd` callback and still does not load NGX, evaluate DLSS, run user rendering, or write visible output.

`Diagnostics.EnableNativeRenderFuncCommandBufferDlssScratchEvaluateProbe=true`, or helper stage `native-renderfunc-commandbuffer-dlss-scratch-evaluate-render-scale`, is the SDK-wrapper-only scratch-output evaluate proof for the same source-guided boundary. It carries focused EASU source/output plus HDRP depth/motion native pointers through one `ctx.cmd` plugin event, creates a native scratch output texture from the visible output descriptor, evaluates DLSS into the scratch texture, and immediately shuts down. It does not write the visible EASU output, and release-safe package builds keep the switch disabled by default.

`Diagnostics.EnableNativeRenderFuncCommandBufferDlssPersistentScratchEvaluateProbe=true`, or helper stage `native-renderfunc-commandbuffer-dlss-persistent-scratch-evaluate-render-scale`, is the SDK-wrapper-only persistent scratch-output evaluate proof for the same source-guided boundary. It carries focused EASU source/output plus HDRP depth/motion native pointers through repeated `ctx.cmd` plugin events, keeps one DLSS frame sequence alive until the target scratch evaluate count succeeds, then releases/destroys/shuts down. It does not write the visible EASU output, and release-safe package builds keep the switch disabled by default.

`Diagnostics.EnableNativeRenderFuncCommandBufferDlssVisibleWritebackProbe=true`, or helper stage `native-renderfunc-commandbuffer-dlss-visible-writeback-render-scale`, is the SDK-wrapper-only visible-output timing/quality proof for the same source-guided boundary. It carries the focused EASU/HDRP frame descriptor through repeated `ctx.cmd` plugin events, keeps one DLSS frame sequence alive until the target visible-output evaluate count succeeds, then releases/destroys/shuts down. It writes the visible EASU output and is for local/private image-correctness testing only; release-safe package builds keep the switch disabled by default.

`Diagnostics.EnableNativeRenderFuncCommandBufferDlssFeatureCreateProbe=true`, or helper stage `native-renderfunc-commandbuffer-dlss-create-render-scale`, is the SDK-wrapper-only follow-up lifecycle proof. It uses the same focused EASU source/output native texture payload and `ctx.cmd` plugin event, then creates and immediately releases/destroys/shuts down one NGX DLSS feature from the native render-event callback. It does not evaluate DLSS or write visible output, and release-safe package builds keep the switch disabled by default.
...
`Diagnostics.EnableRenderScaleControlProbe=true` is the guarded FSR Off render-scale diagnostic. It patches HDRP dynamic-resolution setup/update calls, requests `allowDynamicResolution=true`, forces `GlobalDynamicResolutionSettings.forceResolution=true`, sets a `forcedPercentage` from `DLSS.QualityMode` or `Advanced.RenderScaleOverride`, and selects HDRP `TAAU` as the non-FSR upscaler landmark. It deliberately does not force Unity's internal DLSS pass; the clean-room native bridge still owns DLSS evaluation when `DLSS.EnableDLSS=true`.
