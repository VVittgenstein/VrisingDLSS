...
Current status: experimental scaffold. `DLSS.EnableDLSS=true` starts an unvalidated one-evaluate-per-Unity-frame candidate, but the package is not ready for public gameplay use yet.
...
`DLSS.EnableDLSS=true`, or helper stage `dlss-user-rendering`, is the first normal-user-path candidate. It does not enable the Stage 10A hold/proof loop; it installs the experimental render-scale control probe, keeps one native frame sequence alive, and evaluates at most once per Unity frame after the accepted Super Resolution RenderGraph tuple is available. Release-safe builds without NVIDIA SDK wrapper support are expected to report blocked/fallback rather than crashing.
