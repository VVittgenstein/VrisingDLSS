...
The current blocker is image-correctness, performance, resize/reset, fallback behavior, productionizing render-scale control, and release-boundary validation for the normal-user path. DLSS runtime load, SDK-wrapper init/query, feature create/release, Stage 8A evaluate-input validation, Stage 8B guarded evaluate, Stage 8C output follow-up, Stage 8D persistent repeated evaluate, Stage 8E Super Resolution input sizing, Stage 8F Super Resolution evaluate, Stage 8G Super Resolution persistent repeated evaluate, Stage 9A Super Resolution frame-sequence evaluate, and Stage 10A visible-path write-back candidate now have local proof in V Rising. The guarded diagnostics prove NGX can evaluate against accepted frame resources, one DLSS feature can handle repeated evaluate calls, V Rising can expose a real render-input-smaller-than-output tuple, NGX accepts that SR tuple, one DLSS feature can persist across repeated immediate evaluates on that SR tuple, one DLSS feature can persist across repeated RenderGraph callbacks for that SR tuple, and the selected SR output target can accept 30 repeated evaluates before clean shutdown. `DLSS.EnableDLSS=true` is now wired to an experimental one-evaluate-per-Unity-frame candidate. The v6 FSR Off constructive run proved the candidate can produce and use a mod-owned `960x540 -> 1920x1080` Super Resolution tuple without V Rising's built-in FSR, but it still does not prove normal-user image correctness or performance value.

Latest local evidence shows HDRP RenderGraph methods expose `TextureHandle` entries named `CameraColor`, `CameraDepthStencil`, `Motion Vectors`, and `NormalBuffer`, but ordinary Harmony prefixes see those handles outside a valid RenderGraph resource read scope. The accepted route is passive aggregation of engine-owned `RenderGraphResourceRegistry.GetTexture(TextureHandle&)` callbacks.

The current diagnostic build therefore avoids calling `GetTexture(TextureHandle&)` from method prefixes and instead listens for engine-owned `RenderGraphResourceRegistry.GetTexture(TextureHandle&)` calls with a read-only postfix. In a 75-second scripted local run on 2026-06-05, that route validated `CameraColor`, `Apply Exposure Destination`, `CameraDepthStencil`, and `Motion Vectors` as same-device D3D11 textures at `720x480`. It also has a guarded `Diagnostics.EnableResourceMaterializationProbe` route that observes `RenderGraphResourceRegistry.CreateTextureCallback(...)` after Unity successfully creates a `TextureResource.graphicsResource`. A separate `Diagnostics.EnableDlssPassResourceProbe` route can deliberately test `DLSSPass.GetViewResources` and `DLSSPass.GetCameraResources`, which return the built-in DLSS pass resource textures, but it is default-off and not yet runtime-validated. A `TextureHandle` implicit-conversion hook was tested and rejected because it produced IL2CPP trampoline errors. A declared diagnostic RenderGraph pass also proved too risky: it injected in gameplay and then V Rising crashed in `coreclr.dll` before the pass render function logged. That path remains disabled behind `Diagnostics.EnableRenderGraphDiagnosticPass=false`. Patching compiler-generated existing HDRP render functions also reproduced the same `coreclr.dll` access-violation crash before any render-function scope log, so it is disabled behind `Diagnostics.EnableExistingRenderFuncProbe=false`.

The latest local `dlss-visible-writeback` run on 2026-06-05 passed Stage 10A without a Windows crash event. Evidence: Stage 8E accepted `CameraColor`, `CameraDepthStencil`, and `Motion Vectors` at `426x284` with `Edge Adaptive Spatial Upsampling` at `720x480`, all on the same D3D11 device, for a measured scale of about `1.690x`. Stage 10A then evaluated into `Edge Adaptive Spatial Upsampling` across RenderGraph callbacks, reached `sequenceEvaluates=30` and `evaluateSuccesses=30` with `recreated=no` after the first callback, and release/destroy/shutdown succeeded. Follow-up logs confirmed the same `Edge Adaptive Spatial Upsampling` output remained D3D11-accessible after evaluate.

An earlier paired local/private gameplay comparison used V Rising's built-in FSR Performance mode to force a real Super Resolution tuple at 4K output. Baseline rendering measured about `159.851` average FPS with `131.246` 1% low FPS. The Stage 10A candidate reached `sequenceSuccesses=30/30` on a `1920x1080 -> 3840x2160` tuple and produced a valid 4K screenshot, but the diagnostic hold mode measured only about `45.982` average FPS with `29.982` 1% low FPS because it repeatedly evaluates DLSS for proof. This is not a normal-user rendering path.

A later scripted `dlss-user-rendering` SDK-wrapper smoke run temporarily set V Rising's built-in FSR Performance mode, accepted the same `1920x1080 -> 3840x2160` tuple, and reached `sequenceSuccesses=11100` with `sequenceCreates=1`, no blocked/failed/disabled user-rendering lines, `CrashEventCount=0`, and cleanup restored FSR Off, loader config, and the release-safe native DLL. A follow-up manual 4K paired run captured matching-resolution screenshots and proved the user-rendering candidate can evaluate through the SDK wrapper, but it also exposed that the pre-throttle route evaluated around `24600` times for `1324` presented frames and dropped from about `157.554` FPS baseline to `44.31` FPS candidate. The current candidate therefore throttles `DLSS.EnableDLSS=true` to at most one evaluate per Unity frame before the next visual/performance test.

The v6 `fsr-off-render-scale-1080p-post-update-fraction-v6-20260606` run kept V Rising `FsrQualityMode=Off`, entered the `11111` gameplay fixture at `1920x1080` Windowed, forced the active HDRP dynamic-resolution handler to `0.5`, accepted `CameraColor/CameraDepthStencil/Motion Vectors=960x540` with output `1920x1080`, and reached repeated SDK-wrapper `DLSS user rendering evaluate succeeded` lines with `sequenceCreates=1`. This is the first mod-owned FSR Off render-scale/evaluate proof.

The MVP visual gate is still blocked on a normal-user `dlss-user-rendering` gameplay visual/performance comparison plus matching human image-quality review. Stage 10A comparison evidence remains diagnostic and cannot satisfy that release gate. The remaining work is output selection, jitter/pre-exposure, resize/reset behavior, productionizing render-scale control, mip bias, performance capture, safe fallback, and packaging. V Rising's built-in HDRP dynamic-resolution/upscale symbols, including FSR1 and DLSS-related entries, remain useful as landmarks for low-resolution color, output, depth, motion vectors, and render-scale state. FSR1 itself is not a DLSS dependency or substitute because DLSS Super Resolution still requires valid depth and motion-vector inputs.
