...
The MVP visual gate is still blocked, but the latest normal-user `dlss-user-rendering` paired visual/performance evidence is now a known-regressed route, not the next route to rerun unchanged. The official-flags parity run proved clean DLSS evaluate with `RenderGraph.GetTexture=0`, but still failed performance with low GPU utilization, so a matching human image-quality review cannot make that artifact pass. Do not rerun the same EASU `ctx.cmd` `dlss-user-rendering` candidate unchanged. The protected `hdrp-dlss-contract-bind-render-scale` contract-bind proof has now passed at true `1920x1080` Windowed with `-ProtectSave -SaveName 11111`, no movement keys, final save restore `ChangeCount=0`, `RenderGraphGetTextureCalls=0`, `UserRenderingCandidateStarted=0`, `DlssEvaluateSucceeded=0`, and `EngineOwnedSuperResolutionChainWithHdrpDepthMotionObserved=true`. The next runtime proof is bounded no-write B/C/D cost isolation under the same protected fixture and system-snapshot protocol: B `easu-carrier-only-cost-render-scale`, C `native-renderfunc-commandbuffer-frame-descriptor-d3d11-render-scale`, and D `native-renderfunc-commandbuffer-event-render-scale`. `scripts/test-bounded-no-write-cost-matrix-contract.ps1` guards that mapping without launching or modifying the game. Stage 10A comparison evidence remains diagnostic and cannot satisfy that release gate.
