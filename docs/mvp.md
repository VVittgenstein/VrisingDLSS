...
The current blocker is the first guarded DLSS evaluate path and image-correctness validation. DLSS runtime load, SDK-wrapper init/query, feature create/release, and Stage 8A evaluate-input validation now have local proof.
...
The next concrete MVP step is to wire only this validated input set into the smallest SDK-wrapper DLSS evaluate path, still behind diagnostics and still with `DLSS.EnableDLSS=false` by default. The follow-up gameplay validation must check output selection, image correctness, jitter/pre-exposure, resize/reset behavior, render-scale control, mip bias, and safe fallback. V Rising's built-in HDRP dynamic-resolution/upscale symbols, including FSR1 and DLSS-related entries, remain useful as landmarks for low-resolution color, output, depth, motion vectors, and render-scale state. FSR1 itself is not a DLSS substitute because DLSS Super Resolution still requires valid depth and motion-vector inputs.
