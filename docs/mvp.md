...
The current blocker is image-correctness validation and normal-user rendering integration. DLSS runtime load, SDK-wrapper init/query, feature create/release, Stage 8A evaluate-input validation, Stage 8B guarded evaluate, and Stage 8C output follow-up now have local proof in V Rising. The guarded evaluate path proves that a one-shot NGX evaluate can run against the accepted frame resources, but it still does not prove that `DLSS.EnableDLSS=true` can safely change the game image for normal users.
...
The latest local `dlss-evaluate` run on 2026-06-05 passed Stage 8B and Stage 8C without a Windows crash event. Evidence: `create=0x00000001`, `evaluate=0x00000001`, `release=0x00000001`, `destroy=0x00000001`, and `shutdown=0x00000001`; the follow-up probe then observed the selected `Apply Exposure Destination` output as D3D11-accessible after evaluate, with the same native pointer later appearing under downstream post-process names including `Uber Post Destination`.
