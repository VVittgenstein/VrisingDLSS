...
The next concrete MVP step is screenshot/visual comparison in local/private gameplay to inspect whether repeated DLSS evaluate calls into the selected SR output target are actually visible and image-correct. The measurement plan is tracked in `docs/development/measurement-plan.md`, and `scripts/get-visual-validation-status.ps1` now gates this evidence in release readiness. After that, the remaining work is output selection, jitter/pre-exposure, resize/reset behavior, render-scale control, mip bias, safe fallback, and finally a normal-user `DLSS.EnableDLSS` path. V Rising's built-in HDRP dynamic-resolution/upscale symbols, including FSR1 and DLSS-related entries, remain useful as landmarks for low-resolution color, output, depth, motion vectors, and render-scale state. FSR1 itself is not a DLSS substitute because DLSS Super Resolution still requires valid depth and motion-vector inputs.
