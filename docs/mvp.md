...
The latest paired local/private gameplay comparison used V Rising's built-in FSR Performance mode to force a real Super Resolution tuple at 4K output. Baseline rendering measured about `159.851` average FPS with `131.246` 1% low FPS. The Stage 10A candidate reached `sequenceSuccesses=30/30` on a `1920x1080 -> 3840x2160` tuple and produced a valid 4K screenshot, but the diagnostic hold mode measured only about `45.982` average FPS with `29.982` 1% low FPS because it repeatedly evaluates DLSS for proof. This is not a normal-user one-evaluate-per-accepted-frame rendering path.

A later scripted `dlss-user-rendering` SDK-wrapper smoke run temporarily set V Rising's built-in FSR Performance mode, accepted the same `1920x1080 -> 3840x2160` tuple, and reached `sequenceSuccesses=11100` with `sequenceCreates=1`, no blocked/failed/disabled user-rendering lines, `CrashEventCount=0`, and cleanup restored FSR Off, loader config, and the release-safe native DLL. This proves the experimental `DLSS.EnableDLSS=true` route can drive the native frame-sequence path continuously in local research builds. It still does not prove gameplay image correctness or representative performance because the run did not capture visual output or frame pacing.
