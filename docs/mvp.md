...
The current blocker is image-correctness validation and normal-user rendering integration. DLSS runtime load, SDK-wrapper init/query, feature create/release, Stage 8A evaluate-input validation, Stage 8B guarded evaluate, Stage 8C output follow-up, and Stage 8D persistent repeated evaluate now have local proof in V Rising. The guarded diagnostics prove NGX can evaluate against the accepted frame resources and that one DLSS feature can handle repeated evaluate calls, but they still do not prove that `DLSS.EnableDLSS=true` can safely change the game image for normal users.
