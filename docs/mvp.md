...
The latest local `dlss-persistent-evaluate` full-chain run on 2026-06-05 passed Stage 8A through Stage 9A without a Windows crash event. Evidence: Stage 8E accepted `CameraColor`, `CameraDepthStencil`, and `Motion Vectors` at `426x284` with `Edge Adaptive Spatial Upsampling` at `720x480`, all on the same D3D11 device, for a measured scale of about `1.690x`. Stage 8F evaluated that SR tuple once, Stage 8G evaluated it repeatedly through one feature in one callback, and Stage 9A kept one DLSS feature alive across later RenderGraph callbacks with `recreated=no`, reaching `sequenceEvaluates=3` and `evaluateSuccesses=3` before release/destroy/shutdown. Follow-up logs confirmed the same `Edge Adaptive Spatial Upsampling` output remained D3D11-accessible after evaluate.
