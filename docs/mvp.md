...
The current blocker is Stage 8B runtime validation and image-correctness validation. DLSS runtime load, SDK-wrapper init/query, feature create/release, and Stage 8A evaluate-input validation now have local proof; the guarded evaluate diagnostic is implemented and build-validated, but it has not yet passed in V Rising.
...
The next concrete MVP step is to run the Stage 8B diagnostic in local/private gameplay using the SDK-wrapper native build and a user-supplied DLSS runtime. The follow-up gameplay validation must check output selection, image correctness, jitter/pre-exposure, resize/reset behavior, render-scale control, mip bias, and safe fallback before `DLSS.EnableDLSS=true` can become a normal-user path. V Rising's built-in HDRP dynamic-resolution/upscale symbols, including FSR1 and DLSS-related entries, remain useful as landmarks for low-resolution color, output, depth, motion vectors, and render-scale state. FSR1 itself is not a DLSS substitute because DLSS Super Resolution still requires valid depth and motion-vector inputs.
