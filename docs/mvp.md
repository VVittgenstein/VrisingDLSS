...
The current blocker is image-correctness validation and normal-user rendering integration. DLSS runtime load, SDK-wrapper init/query, feature create/release, Stage 8A evaluate-input validation, Stage 8B guarded evaluate, Stage 8C output follow-up, Stage 8D persistent repeated evaluate, Stage 8E Super Resolution input sizing, Stage 8F Super Resolution evaluate, and Stage 8G Super Resolution persistent repeated evaluate now have local proof in V Rising. The guarded diagnostics prove NGX can evaluate against accepted frame resources, one DLSS feature can handle repeated evaluate calls, V Rising exposes a real render-input-smaller-than-output tuple, NGX accepts that SR tuple, and one DLSS feature can persist across repeated evaluates on that SR tuple, but they still do not prove that `DLSS.EnableDLSS=true` can safely change the game image for normal users.
...
The latest local `dlss-persistent-evaluate` run on 2026-06-05 passed Stage 8B, Stage 8C, Stage 8D, Stage 8E, Stage 8F, and Stage 8G without a Windows crash event. Evidence: one-shot `create/evaluate/release/destroy/shutdown` all returned `0x00000001`; the follow-up probe observed the selected `Apply Exposure Destination` output as D3D11-accessible after evaluate, with the same native pointer later appearing under downstream post-process names including `Uber Post Destination`; the persistent probe then created one feature, ran `evaluateCount=3` with `evaluateSuccesses=3`, and released/destroyed/shutdown successfully. Stage 8E later accepted `CameraColor`, `CameraDepthStencil`, and `Motion Vectors` at `426x284` with `Edge Adaptive Spatial Upsampling` at `720x480`, all on the same D3D11 device, for a measured scale of about `1.690x`. Stage 8F evaluated that SR tuple successfully with `render=426x284`, `target=720x480`, and `evaluate=0x00000001`. Stage 8G then created one feature for the same SR tuple, ran `evaluateCount=3` with `evaluateSuccesses=3`, and released/destroyed/shutdown successfully. Follow-up logs confirmed the same `Edge Adaptive Spatial Upsampling` output remained D3D11-accessible after evaluate.

The next concrete MVP step is to turn that diagnostic evidence into a guarded rendering path that can be enabled and disabled, then validate visible write-back, output selection, image correctness, jitter/pre-exposure, resize/reset behavior, render-scale control, mip bias, and safe fallback in local/private gameplay. V Rising's built-in HDRP dynamic-resolution/upscale symbols, including FSR1 and DLSS-related entries, remain useful as landmarks for low-resolution color, output, depth, motion vectors, and render-scale state. FSR1 itself is not a DLSS substitute because DLSS Super Resolution still requires valid depth and motion-vector inputs.
