...
- Static HDRP/Core source follow-up now points at explicit software fallback rather than another hardware DRS rerun: `DynamicResolutionHandler.ForceSoftwareFallback()` makes `SoftwareDynamicResIsEnabled()` eligible even while the settings type remains Hardware, and `GetResolvedScale()` should then use `m_CurrentFraction` instead of `ScalableBufferManager.widthScaleFactor/heightScaleFactor`. `RenderScaleControlProbe` now invokes `ForceSoftwareFallback()` from the observed `DynamicResolutionHandler.Update(...)` route and logs `SoftwareDynamicResIsEnabled`, `HardwareDynamicResIsEnabled`, `DynamicResolutionEnabled`, `GetCurrentScale`, `GetResolvedScale`, and `ScalableBufferManager` state. The next run is `fsr-off-render-scale-1080p-software-fallback-v5-20260606`.
