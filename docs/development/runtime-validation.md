...
- The first `dlss-user-rendering` release-safe smoke run on 2026-06-05 proved `DLSS.EnableDLSS=true` installs the crash-safe RenderGraph route and starts the one-evaluate-per-frame candidate. With V Rising's built-in FSR Off, the run had no crash event and restored loader config, but no Super Resolution tuple was accepted because `CameraColor` and output were both `3840x2160`.
