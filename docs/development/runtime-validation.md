...
- Current next route: keep Stage 8A's passive builder/resource discovery and `GetTexture` postfix, but do not inject a new RenderGraph pass in normal diagnostics. The active path is the existing HDRP render-function probe, which targets V Rising's built-in dynamic-resolution/upscale/post-process path and treats FSR/TAA/DLSS-related HDRP symbols as landmarks for where low-resolution color, output, depth, and motion-vector resources are already in scope.
