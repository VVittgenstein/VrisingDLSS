...
- Current next route: keep Stage 8A's passive builder/resource discovery and `GetTexture` postfix, but do not inject a new RenderGraph pass or patch compiler-generated render functions in normal diagnostics. Continue using V Rising's built-in dynamic-resolution/upscale/post-process symbols as landmarks, but look for a safer engine-owned resource materialization point.
