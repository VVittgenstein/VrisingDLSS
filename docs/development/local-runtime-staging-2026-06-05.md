...
This records local staging progress for runtime validation. Stage 8B guarded DLSS evaluate, Stage 8C output follow-up, Stage 8D persistent repeated evaluate, and Stage 8E Super Resolution input sizing now have local proof, but normal-user DLSS rendering is not implemented yet.
...
- Keep Stage 8B/8C/8D/8E as guarded diagnostics while `DLSS.EnableDLSS=false` remains the package default.
