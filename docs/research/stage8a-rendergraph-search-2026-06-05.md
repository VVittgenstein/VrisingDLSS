...
Implementation follow-up: a new isolated `Diagnostics.EnableDlssPassResourceProbe=false` route now patches only `DLSSPass.GetViewResources` and `DLSSPass.GetCameraResources`. These helpers are interesting because Unity's source shows they convert HDRP DLSS `TextureHandle` groups into real `Texture` resources before the NVIDIA render path consumes them. The route does not patch the rejected `DLSSPass.Render` method and is not enabled by the ordinary `dlss-evaluate-inputs` helper. A 2026-06-05 main-menu run patched both helpers and produced no matching Windows crash event, but did not observe either helper being called before shutdown. This keeps the route alive as patch-stable initial evidence, not as Stage 8A input evidence.
...
2026-06-06 route correction: later controlled gameplay performance tests proved the
accepted `GetTexture` postfix is viable as a diagnostic tuple oracle but too hot for
steady-state user rendering. The no-evaluate series reproduced severe FPS loss
without native NGX evaluate, and the materialization-only follow-up disabled the
global GetTexture probe but observed `0` materialization SR candidates or tuple
acceptances. A narrow source pass in
`hdrp-dlss-execution-boundary-2026-06-06.md` located the official HDRP boundary as
the `Deep Learning Super Sampling` RenderGraph pass execution window. The next route
is therefore a read-only, narrowly filtered RenderGraph pass-execution proof, not
more global resource lookup or `CreateTextureCallback`-only discovery.

Follow-up Stage 8B/8C/8D/8E/8F/8G/9A/10A evidence: scripted diagnostic runs completed without Windows crash events and successfully created/evaluated/released/destroyed/shutdown DLSS features through the local SDK-wrapper build. The Stage 8C output follow-up observed the selected `Apply Exposure Destination` output pointer as D3D11-accessible in later engine-owned `GetTexture` callbacks, including downstream post-process names such as `Uber Post Destination`. Stage 8D then created one DLSS feature, evaluated it three times successfully, and released/shutdown cleanly. Stage 8E later accepted a Super Resolution-sized tuple: `CameraColor`, `CameraDepthStencil`, and `Motion Vectors` at `426x284` with `Edge Adaptive Spatial Upsampling` at `720x480`. Stage 8F then evaluated that SR tuple successfully with `render=426x284`, `target=720x480`, and `evaluate=0x00000001`. Stage 8G then created one DLSS feature for that SR tuple, evaluated it three times successfully, and released/shutdown cleanly. Stage 9A then kept one DLSS feature alive across three RenderGraph callbacks for that same SR tuple, with later callbacks reporting `recreated=no`, and shut down cleanly. Stage 10A then repeatedly evaluated into the selected SR output target, reached `sequenceEvaluates=30` and `evaluateSuccesses=30`, and shut down cleanly. This still does not prove screenshot-level image correctness, but it moves the route from "can we call evaluate?" to "can we repeatedly evaluate into the likely visible SR output target?"
...
- First persistent repeated Super Resolution-sized DLSS evaluate: validated locally on 2026-06-05 through the SDK-wrapper research build.
- First cross-callback Super Resolution-sized DLSS feature reuse: validated locally on 2026-06-05 through the SDK-wrapper research build.
