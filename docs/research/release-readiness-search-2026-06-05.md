...
- Added Stage 9A frame-sequence evidence. A later 170-second scripted local `dlss-super-resolution-frame-sequence` run on 2026-06-05 had no matching Windows crash event, created one DLSS feature for the accepted SR tuple, reused it across later RenderGraph callbacks with `recreated=no`, reached `sequenceEvaluates=3` and `evaluateSuccesses=3`, and shut down cleanly.
