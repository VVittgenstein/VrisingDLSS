...
- Static HDRP/Core source follow-up now points at explicit software fallback rather than another hardware DRS/handler-request rerun. `DynamicResolutionHandler.ForceSoftwareFallback()` makes `SoftwareDynamicResIsEnabled()` eligible while Hardware settings remain in place, and `GetResolvedScale()` should then use `m_CurrentFraction` instead of `ScalableBufferManager.widthScaleFactor/heightScaleFactor`. `RenderScaleControlProbe` now invokes `ForceSoftwareFallback()` from the observed `DynamicResolutionHandler.Update(...)` route and logs software/hardware DRS state, current/resolved scale, and ScalableBufferManager scale for the next v5 runtime proof.
...
- FSR Off render-scale control has four runtime results, all failed for the MVP tuple: HDRP settings are mutated to 50 percent and RTHandles hardware dynamic-resolution state is requested, but the actual gameplay camera/main render targets remain full-size. The latest direct handler-request run proved `DynamicResolutionHandler.SetCurrentCameraRequest(true)` succeeds and the request is true while `CameraColor` remains full-size. The next minimal diagnostic is explicit `DynamicResolutionHandler.ForceSoftwareFallback()` plus `SoftwareDynamicResIsEnabled`/`GetResolvedScale`/`ScalableBufferManager` readback.
...
5. Resume the technical path from `docs/development/pause-state-2026-06-05.md`:
   ...
   - run `fsr-off-render-scale-1080p-software-fallback-v5-20260606`, the new explicit `DynamicResolutionHandler.ForceSoftwareFallback()` diagnostic, at true `1920x1080` Windowed with `11111` save backup/restore;
   - do not repeat `fsr-off-render-scale-1080p-v1-20260606`, `fsr-off-render-scale-1080p-hwdrs-v2-20260606`, `fsr-off-render-scale-1080p-handler-request-v3-20260606`, or `fsr-off-render-scale-1080p-handler-request-v4-20260606` unchanged; the next loop should expect either a Performance-mode tuple near `960x540 -> 1920x1080` or fallback readback proving whether the scale is applied before/after `HDCamera.GetScaledSize(...)`;
   - reserve 4K/native-output performance comparison for the later controlled final validation matrix.

## Current Repository Checkpoint

As of the software-fallback render-scale follow-up:

- Branch: `main`.
- Latest pushed checkpoint before this update: `f3a28cb Record handler request runtime result`.
- The current working tree records the `fsr-off-render-scale-1080p-handler-request-v4-20260606`
  gameplay result, safe cleanup, save restoration, failed tuple proof, and a follow-up
  explicit software-fallback diagnostic patch.
